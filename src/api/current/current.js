...
module.exports = class Current extends Entity {
  ...
  _handle(config) {
    ...
    return utils
      .slugify(
        this.parent.getProp("prefix")
          ? `${this.parent.getProp("prefix")}-${config.name}`
          : config.name
      )
      .toLowerCase();
  }
  ...
  get notes() {
    return _.get(this._notes, "isFile")
      ? this._notes.getContentSync()
      : this._notes;
  }
  ...
  getPreviewContext() {
    return this.isCollated
      ? this.variants().getCollatedContext()
      : this.variants()
          .default()
          .getResolvedContext();
  }
  ...
  getPreviewContent() {
    return this.isCollated
      ? this.variants().getCollatedContent()
      : this.variants()
          .default()
          .getContent();
  }
      ...
      if (groups) {
        for (const key in groups) {
          ...
          const items = this._resources
            ...
            .items()
            .map(
              file => new Asset(file._file, this.source.relPath, this.source)
            );
  ...
  static *create(config, files, resources, parent) {
    ...
    const comp = new Current(config, files, resources, parent);
