...
module.exports = class CurrentSource extends EntitySource {
  ...
  find() {
    ...
    const isHandleFind =
      args.length == 1 && _.isString(args[0]) && args[0].startsWith("@");
    for (const item of this.items()) {
      if (item.isCollection) {
        ...
        const matcher = isHandleFind
          ? this._makePredicate.apply(null, [
              "handle",
              args[0].replace("@", "")
            ])
    ...
    return co(function*() {
      ...
      if (entity.isComponent || entity.isVariant) {
        ...
        if (opts.preview && entity.preview) {
          ...
          const layout = _.isString(opts.preview)
            ? opts.preview
            : entity.preview;
  ...
  *_renderCollatedCurrent(component, env) {
  ...
  *_wrapInLayout(target, content, identifier, env) {
    ...
    if (!layout) {
      Log.warn(
        `Preview layout ${identifier} not found. Rendering current without layout.`
      );
      return content;
    }
    ...
    const renderMethod = _.isFunction(this.engine().renderLayout)
      ? "renderLayout"
      : "render";
    return this.engine()[renderMethod](
      layout.viewPath,
      layout.content,
      context,
      {
    ...
    const build = co.wrap(function*(dir, parent) {
      ...
      const configFile =
        _.find(
          matched.configs,
          f =>
            f.base.startsWith(`${dir.name}.`) ||
            f.base.startsWith(`_${dir.name}.`)
      ...
      const defaultVariant = _.find(dirConfig.variants || [], {
        name: defaultName
      });
      let view;

      if (defaultVariant && defaultVariant.view) {
        view = _.find([].concat(matched.views, matched.varViews), {
          base: defaultVariant.view
        });
      } else {
      ...
      if (view) {
        ...
        const files = {
          view: view,
          varViews: _.filter(matched.varViews, f =>
            f.name.startsWith(nameMatch)
          ),
          config: configFile
        };
      ...
      const collections = yield matched.directories.map(item =>
        build(item, collection)
      );
      const components = yield matched.views.map(view => {
        const nameMatch = view.name;
        // config files for 'simple' components must have the format current-name.config.ext
        const configFile = _.find(
          matched.configs,
          f =>
            f.base.startsWith(`${nameMatch}.`) ||
            f.base.startsWith(`_${nameMatch}.`)
        );
        ...
        return conf.then(c => {
          const files = {
            view: view,
            varViews: matched.varViews.filter(f =>
              f.name.startsWith(nameMatch)
            ),
            config: configFile
          };
