...
class Fractal extends mix(Configurable, Emitter) {
  ...
  get current() {
    if (!this._current) {
      const CurrentSource = require("./api/current");
      this._current = new CurrentSource(this);
    }
  ...
  _sources() {
    return [this.components, this.current, this.docs].concat(
      this.assets.sources()
    );
  }
...
module.exports.core = {
  ...
  Current: require("./api/current/current.js"),
