...
module.exports = class Builder extends mix(Emitter) {
  ...
  start() {
    ...
    return this._app
      ...
      .then(() => {
        ...
        const setup = fs
          .removeAsync(this._config.dest)
          .then(() => fs.ensureDirAsync(this._config.dest));
        ...
        return setup.then(() => {
          ...
          jobs.push(
            this._static.map(p =>
              this._throttle(() =>
                this._copy(p.path, Path.join(Path.sep, p.mount))
              )
            )
          );
  ...
  _buildRequests() {
    ...
    _.forEach(resolvers, (routeResolvers, handle) => {
      ...
      for (let resolver of routeResolvers) {
        let resolverSet = _.isFunction(resolver)
          ? resolver(this._app)
          : [].concat(resolver);
  ...
  _onError(err, req, dest) {
    ...
    this.emit(
      "error",
      new Error(`Failed to export url ${req.url} - ${err.message}`)
    );
  ...
  _write(contents, dest) {
    dest = _.trimEnd(Path.join(this._config.dest, dest), Path.sep);
    return fs
      .ensureDirAsync(Path.parse(dest).dir)
      .then(() => fs.writeFileAsync(dest, contents));
  }
  ...
  _validate() {
    if (!this._config.dest) {
      throw new Error(
        "You need to specify a build destination in your configuration."
      );
    }
